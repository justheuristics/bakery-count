...
`loadData()` loads `ITEMS_DATA` from `data.json` first. Immediately after, `loadMasterDataFromFB()` runs and **overwrites `ITEMS_DATA` with Firebase's `masterData/items` node if it exists.** In this project's production Firebase, that node already exists and has drifted from `data.json` in both directions (345 live items vs. 373 in the repo as of 31 Aug 2026 — 83 codes exist live but not in the repo, 111 exist in the repo but not live) because admins have been adding items directly through the in-app "➕ เพิ่มรายการสินค้าใหม่" screen (`showAddItemModal`/`doAddItem`/`saveMasterItems`, ~app.js:2784-2880/3009), which writes straight to Firebase and never touches `data.json`, while some repo edits (e.g. adding an item to `data.json` for repo hygiene without also writing it to Firebase) go the other way. Don't assume either file is a superset of the other — diff the two before trusting a count.
